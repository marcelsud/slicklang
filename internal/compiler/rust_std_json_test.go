...
// rustStdJSONProgram exercises the std.json contract the Rust backend must
...
// The malformed input is an empty string so no backtick raw literal is needed;
// the final result is bound to a let so no statement line starts with "(".
const rustStdJSONProgram = `union Shape {
    Circle(Radius: float)
    Square(Side: float)
}
...
// rustStdJSONEdgeProgram covers the encoding/json mismatches: number grammar,
// raw control bytes in strings, unpaired high-surrogate then non-low escape,
// and negative zero.
const rustStdJSONEdgeProgram = `function ReportInt(Text: string) -> string {
...
// rustStdJSONDepthProgram checks that decode accepts nesting well past 256
// (encoding/json's limit is 10000), that a document past that limit is a
// typed Failure, and that encoding a deep value does not abort.
const rustStdJSONDepthProgram = `class Nest {
    Child: Nest?
    Value: int?
}
...
func assertRustJSONMatchesInterpreter(t *testing.T, text string) {
	t.Helper()
	...
	if string(output) != interpreted+"\n" {
		t.Fatalf("Rust std.json output=%q\nerror=%v\nwant interpreter=%q", output, err, interpreted+"\n")
	}
}

func TestRustStdJSONMatchesInterpreter(t *testing.T) {
	t.Run("Contract", func(t *testing.T) {
		assertRustJSONMatchesInterpreter(t, rustStdJSONProgram)
	})
	source := Source{Name: "main.slk", Namespace: "root", Text: rustStdJSONEdgeProgram}
	interpreted, diagnostics, err := Run([]Source{source})
	if err != nil {
		t.Fatalf("run interpreter: %v", err)
	}
	requireNoRustDiagnostics(t, diagnostics)
	binary := buildRustTestProgram(t, source)
	for _, name := range []string{"NumberGrammar", "ControlBytes", "Surrogate", "NegativeZero"} {
		name := name
	...
	depthSource := Source{Name: "main.slk", Namespace: "root", Text: rustStdJSONDepthProgram}
	depthInterpreted, depthDiagnostics, depthErr := Run([]Source{depthSource})
	if depthErr != nil {
		t.Fatalf("run interpreter: %v", depthErr)
	}
	requireNoRustDiagnostics(t, depthDiagnostics)
	depthBinary := buildRustTestProgram(t, depthSource)
	for _, name := range []string{"DeepDecode", "OverLimit", "DeepEncode"} {
		name := name
		t.Run(name, func(t *testing.T) {
			output, err := exec.Command(depthBinary).CombinedOutput()
			if err != nil {
				t.Fatalf("run rust binary: %v: %s", err, output)
			}
			if string(output) != depthInterpreted+"\n" {
				t.Fatalf("Rust std.json output=%q\nerror=%v\nwant interpreter=%q", output, err, depthInterpreted+"\n")
			}
		})
	}
}
