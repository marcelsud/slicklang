...
// bunStdProcessProgram exercises every observable path of std.process.Run:
// captured stdout, a nonzero exit that is still Ok, a missing-program Spawn
// failure, a bad WorkingDirectory failure, and an OutputLimit failure.
const bunStdProcessProgram = `
...
const bunStdProcessOverlayProgram = `
...
const bunStdProcessBareNameProgram = `
...
const bunStdProcessRelativePathProgram = `
function RestorePath(Saved: string?) -> null effects { environment } {
    if (Saved == null) {
        let _ = std.env.Unset("PATH")
        null
...
func assertBunStdProcessMatches(t *testing.T, text, want string) {
	t.Helper()
	...
	if err != nil || string(output) != interpreted+"\n" {
		t.Fatalf("Bun process output=%q error=%v, want %q", output, err, interpreted+"\n")
	}
}

func TestBunStdProcessMatchesInterpreter(t *testing.T) {
	t.Run("Contract", func(t *testing.T) {
		assertBunStdProcessMatches(t, bunStdProcessProgram, "(ok exit=0 out=bytes[6] err=bytes[0], ok exit=1 out=bytes[0] err=bytes[0], err op=Spawn prog=no-such-program-xyz msg=exec: \"no-such-program-xyz\": executable file not found in $PATH, err op=WorkingDirectory prog=echo msg=working directory is not an existing directory, err op=OutputLimit prog=echo msg=captured output exceeds 4 bytes)")
	})

	t.Run("OverlayChild", func(t *testing.T) {
		assertBunStdProcessMatches(t, bunStdProcessOverlayProgram, "visible")
	})
	...
	t.Run("NonExecutableBareName", func(t *testing.T) {
		...
		assertBunStdProcessMatches(t, bunStdProcessBareNameProgram, `Spawn:exec: "slick-not-exec": executable file not found in $PATH`)
	})

	t.Run("NonExecutableThenMissing", func(t *testing.T) {
		denied := t.TempDir()
		empty := t.TempDir()
		if err := os.WriteFile(filepath.Join(denied, "slick-not-exec"), []byte("#!/bin/sh\necho hi\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		t.Setenv("SLICK_TEST_BARE_DIR", denied+string(os.PathListSeparator)+empty)
		assertBunStdProcessMatches(t, bunStdProcessBareNameProgram, `Spawn:exec: "slick-not-exec": executable file not found in $PATH`)
	})
	...
	t.Run("RelativePATHIsRefused", func(t *testing.T) {
		...
		source := Source{Name: "main.slk", Namespace: "root", Text: bunStdProcessRelativePathProgram}
		binary := buildBunTestProgram(t, source)
		t.Chdir(root)
		...
		if err != nil || string(output) != interpreted+"\n" {
			t.Fatalf("Bun relative PATH output=%q error=%v, want %q", output, err, interpreted+"\n")
		}
	})
}
